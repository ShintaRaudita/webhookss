...
func PostBalasan(w http.ResponseWriter, r *http.Request) {
	...
	link := "https://medium.com/@exuvia3021/whatsauth-free-2fa-otp-notif-whatsapp-gateway-api-gratis-e01f1c052c0b"
	if r.Header.Get("Secret") == os.Getenv("SECRET") {
		if msg.Message == "loc" || msg.Message == "Loc" || msg.Message == "lokasi" || msg.LiveLoc {
			...
			dt := &wa.TextMessage{
				To:       msg.Phone_number,
				IsGroup:  false,
				Messages: "Hai Hai " + msg.Alias_name + "\n Shinta lagi gak ada \n aku Shinta Raudita salam kenalll \n Cara penggunaan WhatsAuth ada di link berikut ini ya...\n" + link,
			}
			resp, _ = atapi.PostStructWithToken[atmessage.Response]("Token", os.Getenv("TOKEN"), dt, "https://api.wa.my.id/api/send/message/text")
		}
	} else {
...
func Liveloc(w http.ResponseWriter, r *http.Request) {
	...
	reply := fmt.Sprintf("Hai hai kamu lagi ada di %s \n Koordinatenya : %s - %s\n", location,
